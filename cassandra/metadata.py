...
def group_keys_by_host(session, keyspace, table, keys):
    """
    ...
    for key in keys:
        serialized_key = list(serializer.serialize(pk, cluster.protocol_version)
                              for serializer, pk in zip(serializers, key))
        if len(serialized_key) == 1:
            routing_key = serialized_key[0]
        else:
            routing_key = b"".join(Statement._key_parts_packed(serialized_key))
